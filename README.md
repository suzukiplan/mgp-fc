...
> 上記例で示した `CMP #32` の場合 `a - 32` の演算を行い、aが32未満だとMSBがセット（つまり、carryがセット）され、32以上だとMSBがクリア（つまり、carryがクリア）になります。そして、aが32と同値なら結果は0なのでゼロがセットされ、aが32でなければ結果は非0なのでゼロがクリアされます。
