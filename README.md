...
- 32 <= a なら carry がセット（BCCで分岐できる）
- a < 32 なら carry がクリア（BCSで分岐できる）
...
> 上記例で示した `CMP #32` の場合 `32 - a` の演算を行い、aが32以上だと結果が負数になるのでMSBがセット（つまり、carryがセット）され、32未満だとMSBがクリア（つまり、carryがクリア）になります。そして、aが32と同値なら結果は0なのでゼロがセットされ、aが32でなければ結果は非0なのでゼロがクリアされます。
> _（「少し特殊」なのは、その他の演算と違い「結果の格納」をskipしている点です）_
