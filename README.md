...
6502には「マリオにジャンプを指示」といった高度な命令は存在しませんが、メモリ値の入れ替えのみでそれを実現する必要があります。

しかし、メモリ値をイジるだけではキャラクタが画面に出力されないし音も鳴りません。また、コントローラの入力内容も受け取ることができないように思えます。それらハードウェアの入出力制御は、I/Oポートと呼ばれる特殊なメモリ領域を操作することで実現します。

> Z80や8086などにはI/O入出力制御専用の命令が存在しますが、6502は全てメモリ操作のみでI/O入出力制御を行う点がやや特殊です。
...
初期の頃のファミコンゲームは、40KBという最近のゲームのグラフィック1枚にも満たないデータだけでプログラム、グラフィック、音楽といった全てのデータが全て揃っていたと考えると、当時は凄まじくデータ効率が良かったのだと実感できると思います。

> ファミコン登場時点の時代背景として、半導体のbit単価が今よりも凄まじく高価だったという理由もあるのですが、インターネット経由でゲームをプレイする場面であれば、コンテンツ容量は小さければ小さいほど良いので、エミュレータ技術を活用すればもっと帯域を節約してゲームをプレイできる環境を整えることができます。例えば、JavaScriptでファミコンライクなWebゲームを配信すると、1ゲーム当たりの容量は少なくとも2〜3MB程度（音楽などをつけようものならそれだけで+数MB）掛かるものが、ファミコンならプログラム、グラフィック、音源の全てがたったの40KBで済むので、インフラコストを1/1000以下に抑えることができます。
