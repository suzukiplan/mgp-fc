...
ファミコンにカセットを挿入して電源を投入すると、カセット内のデータ（とプログラム）がメモリ（$0000〜$FFFF）の所定の番地に読み込まれて、所定の番地（メモリの$FFFC, $FFFD番地にリトルエンディアンで格納されているアドレス）からプログラムが動き始めます。そして、プログラムが動いた（処理した）結果の映像と音声がテレビとスピーカに出力されます。
...
6502では、以下4種類の割り込みをサポートしています。

- RESET (リセット割り込み)
- IRQ; Interrupt ReQuest (外部ボード等から送られる割り込み要求 ※Iビットで可能)
- NMI; Non-Maskarable Interrupt (外部ボード等から送られる割り込み要求 ※Iビットでマスク不可)
- BRK (IRQと同じ割り込みをプログラム側から実行する)

> 割り込みに関する知識はゲームを作ってみる段階であれば不要で、実際 [COSMIC SHOOTER](https://github.com/suzukiplan/stg-for-nes) や [BATTLE MARINE](https://github.com/suzukiplan/battle-marine-fc) では（リセット割り込みの対応処理以外）割り込みを使っていません。
> ただし、割り込みを使いこなすことで出来るようになる幅がかなり広がるのも事実です。
> まずは、割り込み無しでゲームを試作してみて、それから習得するのが良いと思います。

割り込みが発生すると、ステータスレジスタと復帰アドレスをスタックに格納しつつ、メモリの以下の領域（割り込みベクタ）にリトルエンディアンで格納されているアドレス値へ分岐します。分岐後は RTI 命令を発行することで元の処理に復帰できます。
...
<img src="images/attr-layout.png"/>

- bit 7〜6 : 右下2x2マスのパレット番号（%00, %01, %10, %11）
- bit 5〜4 : 左下2x2マスのパレット番号（%00, %01, %10, %11）
- bit 3〜2 : 右上2x2マスのパレット番号（%00, %01, %10, %11）
- bit 1〜0 : 左上2x2マスのパレット番号（%00, %01, %10, %11）
